...
function CarLeasingApplication({}) {
  ...
  const [initialFeeInPercentages, setInitialFeeInPercentages] = React.useState(10);
  const [leasingTerm, setLeasingTerm] = React.useState(1);
  const [amountOfLeasingAgreement, setAmountOfLeasingAgreement] = React.useState(0);
  ...
  React.useEffect(() => {
    setInitialFeeInPercentages(carCost ? Math.round((initialFee / carCost) * 100) : 0);
  }, [initialFee]);
  ...
  React.useEffect(() => {
    setMonthlyPayment(
      Math.round(
        (carCost - initialFee) *
          ((0.035 * Math.pow(1 + 0.035, leasingTerm)) / (Math.pow(1 + 0.035, leasingTerm) - 1)),
      ),
    );
  ...
  React.useEffect(() => {
    setAmountOfLeasingAgreement(Math.round(initialFee + leasingTerm * monthlyPayment));
  }, [initialFee, leasingTerm, monthlyPayment]);

  // Calculate initialFee when has been changed carCost
  React.useEffect(() => {
    if (carCost >= 1000000) {
      setInitialFee(Math.round(((initialFeeInPercentages || 10) / 100) * carCost));
    } else {
      setInitialFee(
        initialFeeInPercentages ? Math.round((initialFeeInPercentages / 100) * carCost) : 0,
      );
    }
  }, [carCost]);
  ...
  return (
    <form className={styles.form} onSubmit={sendApplication}>
      <Header1 className={styles.header} text="Рассчитайте стоимость автомобиля в лизинг" />
      ...
      <div className={styles.footerAlign}>
        <div className={styles.alignCalculations}>
          <CalculationResult title="Сумма договора лизинга" result={amountOfLeasingAgreement} />
          <CalculationResult title="Ежемесячный платеж от" result={monthlyPayment} />
        </div>

        <StyledButton
          className={styles.submitFormButton}
          isLoading={!applicationIsLeft}
